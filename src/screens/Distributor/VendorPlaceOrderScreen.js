...
export default function VendorPlaceOrderScreen({route,navigation}) {
    ...
    return (
     ...
     <View style={styles.container}>
        <View style={styles.titleBar}>
          ...
          <TouchableOpacity onPress={()=>navigation.navigate('VendorCheckoutScreen',{data:cart})}>
            <BadgeIcon
              name="shopping-cart"
              size={35}
                    ...
                    <TouchableOpacity onPress={()=>navigation.navigate('VendorProductsDetails',{data:item})}>
                        <View style={styles.boxStyle}>
                           ...
                           <View style={{marginLeft:5,marginTop:5}}>
                             ...
                             onPress={()=>{
                                if(item.qtyOdered!=0 && item.qtyOdered>=item.thresholdToBuy && (cart.filter(c=>c.pid===item.pid)).length===0){
                                    //console.log(item)
                                    //console.log(cart)
                                   // setCart({...cart,item})
